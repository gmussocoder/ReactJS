...
const Checkout = () => {
    ...
    const updateStockAndSales = () => {
        cart.cartList.forEach((itemCart) => {
            UpdateDB(itemCart.item.id, "gmussocollection", {"stock": increment(parseInt(itemCart.quantity)*-1)})
                .then(result => {})
                .catch(error => {
                   console.log(error);
                   setModal({"title": "Que lástima! hubo un error!", "body": "Lo lamentamos, tu pedido ha sido descartado debido a un error."})
                   handleShow();
                   return;
                });
            UpdateDB(itemCart.item.id, "gmussocollection", {"sales": increment(parseInt(itemCart.quantity))})
                .then(result => {})
                .catch(error => {
                   console.log(error);
                   setModal({"title": "Que lástima! hubo un error!", "body": "Lo lamentamos, tu pedido ha sido descartado debido a un error."})
    ...
    const handleSubmit = (event) => {
        ...
        InsertDB(doc, "gmussocollectionorders")
            .then(result => {
               setModal({"title": "Tu Pedido se ha realizado!", "body": "Almacena tu número de pedido por cualquier consulta en adelante: " + result.id});
               updateStockAndSales();
               handleShow();
            })
            .catch(error => {
               console.log(error);
               setModal({"title": "Que lástima! Hubo un error!", "body": "Lo lamentamos, tu pedido ha sido rechazado debido a un error."})
               cart.clear();
               handleShow();
               return;
            });
